...
NBT Viewer is one mod built for **22 Minecraft versions, 1.20 → 26.3**, from a **single Stonecutter
...
| Mojmap | **26.1, 26.2, 26.3** | Mojang (official names) | `net.fabricmc.fabric-loom` 1.17.20 | 25 | `build.fabric26.gradle` |
...
## How one source serves all 22 versions
...
| `Keys` | GLFW key codes before 26.3 → vanilla SDL scancodes and modifier masks on 26.3+ |
...
| **26.3** | SDL replaces GLFW; key bindings use `InputConstants.Type.KEYBOARD`, and `Keys` supplies the matching scancodes and modifier masks |
...
JAVA_HOME=/path/to/jdk-25 ./gradlew :26.3-fabric:build
